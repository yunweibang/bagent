...
func (p *program) run() {
	...
	serverAddr := ""
	if conf.ProxyIP != "" && conf.ProxyPort != "" {
		serverAddr = "http://" + conf.ProxyIP + ":" + conf.ProxyPort
	}
	wg := sync.WaitGroup{}
	for {
		wg.Add(1)
		go func() {
			resp, err := r.Post(serverAddr+"/agent/tasklist", req.Param{
				"id":       conf.HostID,
				"ak":       conf.HostAK,
		...
		go func() {
			resp, err := r.Post(serverAddr+"/agent/netstat", req.Param{
				"id":      conf.HostID,
				"ak":      conf.HostAK,
				"type":    "ip",
		...
		wg.Add(1)
		go func() {
			resp, err := r.Post(serverAddr+"/agent/cron", req.Param{
				"id":           conf.HostID,
				"ak":           conf.HostAK,
		...
		wg.Add(1)
		go func() {
			resp, err := r.Post(serverAddr+"/agent/version", req.Param{
				"id":            conf.HostID,
				"ak":            conf.HostAK,
		...
		wg.Add(1)
		go func() {
			resp, err := r.Post(serverAddr+"/agent/zabbixagent/version", req.Param{
				"id":                conf.HostID,
				"ak":                conf.HostAK,
